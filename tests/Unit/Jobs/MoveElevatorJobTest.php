...
class MoveElevatorJobTest extends TestCase
{
    ...
    public function test_it_handles_upwards_and_downward_movement()
    {
        // Fake the Queue for testing
        Queue::fake();
        ...
        // Create a new elevator log to call the elevator to the 3rd floor
        $newLogTo3rdFloor = ElevatorLog::factory()->create([
            'elevator_id' => $elevator->id,
            'current_floor' => $elevator->logs()->latest()->first()->current_floor,
            'state' => $elevator->logs()->latest()->first()->state,
            'action' => 'call',
        ...
        // Execute the second movement job
        $jobTo3rdFloor = new MoveElevator($newLogTo3rdFloor);
        $jobTo3rdFloor->handle();

        // Fetch the latest logs
        $elevatorLogs = $elevator->logs()->get();

        // Assert the elevator's states match the provided sequence
        $expectedStates = [
            'idle',
            'idle',
            'moving',
        ...
        foreach ($expectedStates as $index => $expectedState) {
            $this->assertEquals($expectedState, $elevatorLogs[$index]->state);
        }
    }
}